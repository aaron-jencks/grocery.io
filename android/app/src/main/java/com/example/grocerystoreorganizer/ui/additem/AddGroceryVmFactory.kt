...
import com.example.grocerystoreorganizer.BuildConfig
...
import com.example.grocerystoreorganizer.data.remote.repository.GrpcPriceObservationRepository
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        val groceryRepo: PriceObservationCrudRepository =
            if (BuildConfig.USE_REMOTE_DB) {
                GrpcPriceObservationRepository(
                    host = BuildConfig.GRPC_HOST,
                    port = BuildConfig.GRPC_PORT,
                )
            } else {
